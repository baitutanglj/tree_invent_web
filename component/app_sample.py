...
from .common_layout import train_dict, sample_constrain_dict, card_style, card_style_hide, \
    form_style, number_style, getter_value
...
sample_dict.update({'sample_constrain': sample_constrain_dict})
rl_dict = {
    'score_components': [],
    'score_weights': [],
    'acc_steps': 1,
...
similarities_dict = {'rl': {'target_smiles': '', 'target_molfile': ''}}
activity_dict = {'rl': {'qsar_models_path': ''}}
...
def set_score_components(score_name, score_layout, score_dict):
    components = fac.AntdFormItem([
        fac.AntdCheckbox(id=f"{score_name}-checkbox", label=score_name, style={'width': '100px'}),
...
rl_value_card = fac.AntdCollapse(
    ...
    children=fac.AntdForm(
        ...
        children=[
            ...
            # score_components,
            fac.AntdDivider('score_components'),
            fac.AntdFormItem([set_score_components(score_name, score_layout, score_dict) for score_name, score_layout, score_dict in
                              zip(score_func_name, score_layout_list, score_data_dict)]),
...
def update_sample_value(rl_nClicks, use_rl_nClicks, similarities_checkbox, activity_checkbox, shape_checkbox, dockscore_checkbox,
    ...
    if use_rl_nClicks and rl_nClicks:
        if similarities_checkbox or activity_checkbox or shape_checkbox or dockscore_checkbox:
            ...
            return data, fac.AntdMessage(content='Submit Successfully!', type='success')
        else:
